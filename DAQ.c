...
int main( void ) {
  ...
  while(1) {
    ...
    for(int i=0 ; i<MAX_CHAN; i++ ){
      ...
      writer.writeSensorData(testNumber,&tv1,i,chan_data[i],count/*func[i].callFunction(chan_voltage[i])*/);
      for (size_t j = 0; j < 10; j++) {
      writer.writeSensorData(testNumber,&tv1,i,chan_data[i],func[i].callFunction(chan_voltage[i]));
      }
